...
debian-base82:
	docker build --platform=linux/arm64 -f "base/8.2/debian.dockerfile" -t $(NAME)-$(VERSION) .
	@if [ "${PUSH}" == 'true' ]; then \
...
debian-base-xdebug82:
	docker build --platform=linux/arm64 -f "base-xdebug/8.2/debian.dockerfile" -t $(NAME)-$(VERSION) .
	@if [ "${PUSH}" == 'true' ]; then \
	    docker push $(NAME)-$(VERSION) ; \
	fi
